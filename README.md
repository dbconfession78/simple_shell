...
GOS works similary to the bourne again shell. Extensive user documentation on specific commands is available by running help within GOS.

GOS builds successfully with a GCC 4.8.5 compiler or later. HSH can compile using `gcc 4.8.4 -Wall -Werror -Wextra -pedantic *.c -o simple_shell`.

To start up GOS, simply type in 
```
./simple_shell
``` 
the terminal.
...
Above will run the `echo` command, which will write its arguements `Hello` to  the standard output. 
